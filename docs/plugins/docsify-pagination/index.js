...
function matches(el, selector) {
  ...
  var nodes = queryAll(selector, el.parentNode);
...
function pagination (vm, { crossChapter }) {
  try {
    ...
    const all = toArray(queryAll('.sidebar-nav li a')).filter((element) => !matches(element, '.section-link'))
    ...
    console.log('Fail parse section', error)
    return {
      route: {}
    }
  }
